...
4. OFF 排除後表面；ON 設定透明厚基板的非相干多次反射。若軟體版本無法表達相同假設，記錄不相容並停止將其稱為等條件 benchmark。
5. 保存 Macleod 版本、結構／材料檔、設定截圖、匯出精度及原始匯出檔。可直接上傳 File → Export → Comma separated unformatted values 的 Performance Table CSV；也可使用 `wavelength,R,T` 有表頭 CSV。Native 匯出的 Phase 欄保留於原始資料，但不進入 R/T 比較。
6. 明確指定 wavelength unit nm 或 um，以及 R/T fraction 或 percent。Native header 單位與選項衝突時會拒絕，不自動猜測。Importer 保存原始檔名、bytes、欄名、映射、原順序、單位與 SHA256；另建 ascending nm/fraction copy。Comparison 報告記錄 alignment method。
